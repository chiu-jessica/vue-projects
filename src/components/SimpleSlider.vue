<template>
    <div class="slider">
      <div class="slider__wrapper">
        <div v-if="!hideLabel" :style="{ left: position }" class="slider__label">{{ labelSymbol == "" ? "" : labelSymbol }} {{ sliderLabel }}</div>
        <div class="slider__track" :class="{'slider__track--rectangular': !raising}" :style="{ background : 'linear-gradient(to right, '+ trackColor +' 0%, '+ trackColor +' ' + percent + '%, '+ trackBgColor +' ' + percent + '%, '+ trackBgColor +' 100%)' }">
          <div
            v-if="raising"
